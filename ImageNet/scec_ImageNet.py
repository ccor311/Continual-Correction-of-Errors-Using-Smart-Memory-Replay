#Code for running SCEC with ILSVRC2012 (ImageNet)
...
# ==================================================
test_type = "new_scaled_decay_imagenet_EC" + ".txt"
...
#SCEC: Define minimum and maximum number of error correction instances, as a proportion of the total number of misclassified instances for each task
min_ec = 0.05
max_ec = 0.20
    ...
    with tf.Session(graph=g1, config=config) as sess1:
        ...
        for j in range(0, task_num):
            ...
            for i_val in range(j + 1):
                ...
                #SCEC: Create list of accuracies for each task
                acc_list[i_val] = accu
            ...
            #SCEC: Start selecting once over 10 tasks learned
            if j > 10:
                task_forgetting = [a - b for a, b in zip(prev_acc_list, acc_list[:-1])]
                k = len(task_forgetting)
                #Sorts forgetting from lowest to highest
                idx = np.argpartition(task_forgetting, k - 1)
                max_forgetting = max(task_forgetting)
                min_forgetting = min(task_forgetting)
                for n in range(k):
                    percent_to_sample = task_forgetting[n] / max_forgetting * max_ec
                    if percent_to_sample < min_ec:
                        percent_to_sample = min_ec
                    dict_incorrect_valid[n] = (np.random.choice(dict_incorrect_valid[n][0], (math.ceil(dict_incorrect_valid[n][0].size * percent_to_sample)), replace=False),)
            #SCEC: update accuracies to new task
