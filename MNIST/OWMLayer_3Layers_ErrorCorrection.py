...
class OWMLayer(object):
    ...
    def __init__(self, shape_list, seed_num=0):
        ...
        #begin DBP: receive input_x_incorrect_validation and input_y_incorrect_validation as examples to calculate Q
        ...
        with tf.name_scope("input_owm"):
            ...
            #output in the l-1 layer in response to the mean inputs (in this case, input_x)
            r = tf.reduce_mean(y_1, 0, keep_dims=True)
            #this serves for the second part of Eq. 2
            k = tf.matmul(self.P1, tf.transpose(r))
            #in Eq. 2 of methods, dela_P1 is the k part
            self.dela_P1 = tf.divide(tf.matmul(k, tf.transpose(k)), self.alpha_array[0][0] + tf.matmul(r, k))
            #this is P_{l}(i -1, j) - k...
        ...
        # DBP: Correct/Incorrect Validation Examples from Previous Tasks
